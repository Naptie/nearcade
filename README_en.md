...
# Optional: restore sanitized seed data (shops, universities, regions, etc.)
pnpm dev:setup --seed
```
...
from a development database into sanitized seed data, then restored into the
local MongoDB:

```bash
# Export from a development database. Run on the host — this is an external
# data source, unrelated to the local Compose network. Specify the source via
# the SEED_SOURCE env var or the --source argument.
...
`seed:dump` sanitization is intentionally minimal — it only strips:

- **Image references**: `universities.avatarUrl`, `universities.avatarImageId`
  (point at OSS that isn't available locally);
- **Ownership/environment fields**: `shops.isClaimed` (references a user
  account that won't exist locally);
- Each collection's `_id` (regenerated by MongoDB on insert).

`createdAt`/`updatedAt` timestamps are **preserved**; `counters.seq` (the next
shop id) is also **preserved** so new shops don't collide with seeded ids.

Seed files are written to `data/seed/` (gitignored, not committed).
