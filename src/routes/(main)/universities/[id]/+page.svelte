<script lang="ts">
  ...
  let hasMoreMembers = $derived(
    (universityDataResolved?.members?.length || 0) >= PAGINATION.PAGE_SIZE
  );
  ...
  let userPrivileges = $derived.by(() => {
    ...
    return (
      universityDataResolved?.userPermissions || { canEdit: false, canManage: false, canJoin: 0 }
    );
  });

  // Check if user can write posts based on university postWritability setting
  let canWritePosts = $derived(
    universityDataResolved
      ? canWriteUnivPosts(
          universityDataResolved?.userPermissions,
          universityDataResolved?.university
        )
      : false
  );
  ...
  const loadMoreMembers = async () => {
    if (isLoadingMoreMembers || !hasMoreMembers || !universityDataResolved) return;
    ...
    try {
      ...
      const response = await fetch(
        fromPath(
          `/api/universities/${universityDataResolved.university.id}/members?page=${nextPage}`
        )
      );
      if (response.ok) {
  ...
  const loadMoreClubs = async () => {
    if (isLoadingMoreClubs || !hasMoreClubs || !universityDataResolved) return;
  ...
  const handleRoleAction = async (action: string, memberId: string) => {
    if (!memberId || !universityDataResolved) return;
  ...
  const canManageMember = (member: UniversityMemberWithUser) => {
    ...
    const isCurrentUserModerator =
      universityDataResolved?.userPermissions?.role === 'moderator' || false;
  ...
  <div class="mx-auto max-w-7xl px-4 pt-20 pb-8 sm:px-6 lg:px-8">
    <div class="py-12 text-center">
      ...
      <p class="text-base-content/60 mb-4">
        {universityDataError.message || m.error_occurred()}
      </p>
  ...
  >
    ...
    <div class="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div class="flex flex-col items-center gap-6 sm:flex-row">
        ...
        <!-- University Info -->
        <div
          class="flex flex-1 flex-col items-center justify-between gap-3 sm:flex-row {universityDataResolved
            ...
            {#if !data.user || (universityDataResolved?.userPermissions.canJoin > 0 && !universityDataResolved?.userPermissions.verificationEmail)}
              <a
                href={resolve('/(main)/universities/[id]/verify', {
                  id:
                    universityDataResolved?.university.slug || universityDataResolved?.university.id
                })}
                class="btn btn-ghost"
              >
            ...
            <div class="space-y-6">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  ...
                  {#if userPrivileges.canManage}
                    <a
                      href="{resolve('/(main)/clubs/new')}?university={universityDataResolved
                        .university.id}"
                            ...
                            <VerifiedCheckMark
                              href={member.userId === data.user?.id
                                ? resolve('/(main)/universities/[id]/verify', {
                                    id:
                                      universityDataResolved?.university.slug ||
                                      universityDataResolved?.university.id
                                  })
                                : ''}
