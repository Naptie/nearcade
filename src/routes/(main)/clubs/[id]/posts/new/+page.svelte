<script lang="ts">
  ...
  let backUrl = $derived(
    resolve('/(main)/clubs/[id]', { id: data.club.slug || data.club.id }) + '#posts'
  );

  const handleCreated = (postId: string) => {
    goto(
      resolve('/(main)/clubs/[id]/posts/[postId]', {
        id: data.club.slug || data.club.id,
        postId
      })
    );
  };
...
<svelte:head>
  <title>{pageTitle(m.create_post(), data.club.name)}</title>
...
>
  ...
  <article class="bg-base-100 rounded-2xl p-6 shadow">
    <PostCreateForm
      ...
      organizationReadability={data.club.postReadability ?? PostReadability.PUBLIC}
