<script lang="ts">
  ...
  const loadNotifications = async (page = 1, append = false) => {
    ...
    try {
      ...
      const response = await fetch(
        `${base}/api/users/${userId}/notifications?page=${page}&limit=20${unreadParam}`
      );
  ...
  const markAsRead = async () => {
    try {
      const userId = data.user.name ? `@${data.user.name}` : data.user.id;
      const response = await fetch(`${base}/api/users/${userId}/notifications`, {
        method: 'POST',
        headers: {
    ...
    {:else if notifications.length > 0}
      ...
      {#if hasMore}
        <div class="mt-6 text-center">
          <button class="btn btn-outline" onclick={loadMoreNotifications} disabled={isLoadingMore}>
            {#if isLoadingMore}
