...
export default defineOpenApiRoute({
  get: {
    ...
    responses: {
      '200': jsonResponse(bilingual('队列数据', 'Queue data', true), queueListResponseOpenApiSchema),
  ...
  post: {
    ...
    description: bilingual('上报店铺中一个或多个游戏的机台队列状态。', 'Report machine queue states for one or more games in a shop.'),
    security: bearerAuth,
    requestParams: {
      path: shopIdParamSchema
    },
    ...
    responses: {
      '200': jsonResponse(bilingual('队列上报成功', 'Queue report accepted', true), queueReportResponseOpenApiSchema),
      '400': { description: bilingual('请求错误', 'Bad Request', true) },
      '401': { description: bilingual('机台凭据无效', 'Invalid machine credentials', true) },
      '403': { description: bilingual('机台未绑定到该店铺', 'Machine not bound to this shop', true) },
