<script lang="ts">
  ...
  // Signed-in users manage their locale from settings (stored in the
  // database, consistent across devices) — the quick switcher is hidden.
  const isLoggedIn = $derived(Boolean(page.data.session?.user));
