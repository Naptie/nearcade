...
<div class="flex {minHeight} flex-col {showPreviewTab ? 'sm:flex-row' : ''}">
  {#if !showPreviewTab || !showPreview}
    <textarea
      {placeholder}
      class="textarea textarea-bordered h-auto w-auto flex-1 resize-none rounded-2xl {showPreviewTab ? 'not-sm:rounded-b-none sm:rounded-r-none' : ''}"
      bind:value
      {disabled}
      onkeydown={onKeyDown}
    ></textarea>
  ...
  {#if showPreviewTab && (showPreview || !showPreview)}
    <div
      class="bg-base-200 prose prose-sm h-auto flex-1 overflow-y-auto rounded-2xl px-4 py-2 {showPreview ? '' : 'not-sm:rounded-t-none sm:rounded-l-none'} {showPreview && showPreviewTab ? 'block' : showPreviewTab ? 'hidden sm:block' : 'hidden'}"
    >
      {#if preview}
        {@html preview}
      {:else}
