...
import type { GitHubEmail, GitHubProfile } from '@auth/sveltekit/providers/github';
...
export default function GitHub(
  ...
  const originalBaseUrl = config?.enterprise?.baseUrl ?? 'https://github.com';
  const baseUrl = env.GITHUB_PROXY
    ? env.GITHUB_PROXY.endsWith('/')
      ? env.GITHUB_PROXY.slice(0, -1)
      : env.GITHUB_PROXY
    : originalBaseUrl;
  const apiBaseUrl = env.GITHUB_API_PROXY
    ? env.GITHUB_API_PROXY.endsWith('/')
      ? env.GITHUB_API_PROXY.slice(0, -1)
  ...
  return {
    ...
    type: 'oauth',
    authorization: {
      url: `${originalBaseUrl}/login/oauth/authorize`,
