...
const commentVoteTypeSchema = z
  .enum(['upvote', 'downvote'])
  .describe(bilingual('评论投票类型。', 'Comment vote type.'));

const imageAssetIdSchema = z.string().describe(bilingual('图片资源 ID。', 'Image asset ID.'));

const imageStorageProviderSchema = z
  .enum(['s3', 'leancloud'])
...
export const imageAssetSchema = z
  .object({
    ...
    shopId: shopIdSchema.optional().describe(bilingual('关联店铺 ID。', 'Associated shop ID.')),
    ...
    deleteRequestId: shopDeleteRequestIdSchema
      .optional()
      .describe(bilingual('关联删除申请 ID。', 'Associated delete request ID.')),
    userId: userIdSchema
      .nullable()
...
export const commentVoteSchema = z
  .object({
    ...
    commentId: z.string().describe(bilingual('评论 ID。', 'Comment ID.')),
    userId: userIdSchema,
