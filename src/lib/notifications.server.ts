...
export async function getUserNotifications(
  ...
  if (notificationTypes.includes('COMMENTS')) {
    const userPosts = await db.collection<Post>('posts').find({ createdBy: userId }).toArray();
    ...
    if (postIds.length > 0) {
      matchConditions.push({
        $and: [
          { postId: { $in: postIds } },
          { createdBy: { $ne: userId } }, // Exclude user's own comments
    ...
    if (commentIds.length > 0) {
      matchConditions.push({
        $and: [
          ...
          { createdBy: { $ne: userId } } // Exclude user's own replies
        ]
      });
    }
  }
  ...
  // Get vote notifications if enabled
  if (notificationTypes.includes('POST_VOTES') || notificationTypes.includes('COMMENT_VOTES')) {
    // Post votes
